...
struct file_view {
  ...
  unsigned int
      protect; /* protection for all pages at allocation time and SEC_* flags */
};
...
#define VPROT_SYSTEM                                                           \
  0x0200 /* system view (underlying mmap not under our control) */
...
static void *address_space_limit =
    (void *)0x7fffffff0000; /* top of the total available address space */
...
#define VIRTUAL_DEBUG_DUMP_RANGES()                                            \
  do {                                                                         \
    if (TRACE_ON(virtual_ranges))                                              \
      dump_free_ranges();                                                      \
  } while (0)
...
static BOOL
    force_exec_prot; /* whether to force PROT_EXEC on all PROT_READ mmaps */
static BOOL enable_write_exceptions; /* raise exception on writes to executable
                                        memory */
...
static inline BOOL is_beyond_limit(const void *addr, size_t size,
                                   const void *limit) {
...
void *anon_mmap_fixed(void *start, size_t size, int prot, int flags) {
  return mmap(start, size, prot, MAP_PRIVATE | MAP_ANON | MAP_FIXED | flags, -1,
              0);
}
...
static void mmap_add_reserved_area(void *addr, SIZE_T size) {
  ...
  if (!((intptr_t)addr + size))
    size--; /* avoid wrap-around */
  end = (char *)addr + size;
  ...
  LIST_FOR_EACH(ptr, &reserved_areas) {
    ...
    while ((next = list_next(&reserved_areas, ptr))) {
      struct reserved_area *area_next =
          LIST_ENTRY(next, struct reserved_area, entry);
      void *next_end = (char *)area_next->base + area_next->size;

      if (area_next->base > end)
        break;
      list_remove(next);
      free(area_next);
      if (next_end >= end) {
...
static void mmap_remove_reserved_area(void *addr, SIZE_T size) {
  ...
  while (ptr) {
    area = LIST_ENTRY(ptr, struct reserved_area, entry);
    if ((char *)area->base >= (char *)addr + size)
      ...
      if (area->base >= addr) {
        ...
        if ((char *)area->base + area->size > (char *)addr + size) {
          ...
          if (new_area) {
            ...
            new_area->size =
                (char *)area->base + area->size - (char *)new_area->base;
 ...
 *
 * Unmap memory that's above the user space limit, by replacing it with an empty
 * mapping, and return the remaining size below the limit. virtual_mutex must be
 * held by caller.
 */
static size_t unmap_area_above_user_limit(void *addr, size_t size) {
  ...
  if (addr < user_space_limit) {
    ...
    if (ret >= size)
      return size; /* nothing is above limit */
    size -= ret;
...
static void *anon_mmap_tryfixed(void *start, size_t size, int prot, int flags) {
  ...
  ptr = mmap(start, size, prot,
             MAP_FIXED_NOREPLACE | MAP_PRIVATE | MAP_ANON | flags, -1, 0);
#elif defined(MAP_TRYFIXED)
  ptr = mmap(start, size, prot, MAP_TRYFIXED | MAP_PRIVATE | MAP_ANON | flags,
             -1, 0);
#elif defined(__FreeBSD__) || defined(__FreeBSD_kernel__)
  ptr = mmap(start, size, prot,
             MAP_FIXED | MAP_EXCL | MAP_PRIVATE | MAP_ANON | flags, -1, 0);
  if (ptr == MAP_FAILED && errno == EINVAL)
    errno = EEXIST;
#elif defined(__APPLE__)
  mach_vm_address_t result = (mach_vm_address_t)start;
  kern_return_t ret =
      mach_vm_map(mach_task_self(), &result, size, 0, VM_FLAGS_FIXED,
...
static void reserve_area(void *addr, void *end) {
  ...
  while (address < end_address) {
    ...
    ret = mach_vm_region(mach_task_self(), &address, &size,
                         VM_REGION_BASIC_INFO_64, (vm_region_info_t)&info,
                         &count, &dummy_object_name);
    if (ret != KERN_SUCCESS) {
      address = max_address;
      size = 0;
    }
    ...
    if (hole_address < address) {
      ...
      ret = mach_vm_map(mach_task_self(), &alloc_address, hole_size, 0,
                        VM_FLAGS_FIXED, MEMORY_OBJECT_NULL, 0, 0, PROT_NONE,
                        VM_PROT_ALL, VM_INHERIT_COPY);
      if (!ret)
        mmap_add_reserved_area((void *)hole_address, hole_size);
...
static void mmap_init(const struct preload_info *preload_info) {
  ...
  if (preload_info) {
    ...
    for (i = 0; preload_info[i].size; i++) {
      if ((char *)preload_info[i].addr > user_space_limit)
        break;
      if ((char *)preload_info[i].addr + preload_info[i].size >
          user_space_limit) {
  ...
  if (stack_ptr >= user_space_limit) {
    char *end = 0;
    char *base = stack_ptr - ((unsigned int)stack_ptr & granularity_mask) -
                 (granularity_mask + 1);
    if (base > user_space_limit)
      reserve_area(user_space_limit, base);
    base = stack_ptr - ((unsigned int)stack_ptr & granularity_mask) +
           (granularity_mask + 1);
...
#ifdef __aarch64__
  reserve_area((void *)0x00007e000000, (void *)0x00007fff0000);
#else
...
static void add_builtin_module(void *module, void *handle) {
  ...
  if (!(builtin = malloc(sizeof(*builtin))))
    return;
...
static void release_builtin_module(void *module) {
  ...
  LIST_FOR_EACH_ENTRY(builtin, &builtin_modules, struct builtin_module, entry) {
    ...
    if (!--builtin->refcount) {
      ...
      if (builtin->handle)
        dlclose(builtin->handle);
      if (builtin->unix_handle)
        dlclose(builtin->unix_handle);
 ...
 */
static NTSTATUS get_builtin_unix_funcs(void *module, BOOL wow,
                                       const void **funcs) {
  const char *ptr_name =
      wow ? "__wine_unix_call_wow64_funcs" : "__wine_unix_call_funcs";
  sigset_t sigset;
...
NTSTATUS load_builtin_unixlib(void *module, const char *name) {
  ...
  LIST_FOR_EACH_ENTRY(builtin, &builtin_modules, struct builtin_module, entry) {
    if (builtin->module != module)
      continue;
    if (!builtin->unix_path)
      builtin->unix_path = strdup(name);
    else
 ...
 *
 * Returns the first range whose end is not less than addr, or end if there's
 * none.
 */
...
static void free_ranges_insert_view(struct file_view *view) {
  ...
  void *view_end = ROUND_ADDR(
      (char *)view->base + view->size + granularity_mask, granularity_mask);
  struct range_entry *range = free_ranges_lower_bound(view_base);
  struct range_entry *next = range + 1;

  /* free_ranges initial value is such that the view is either inside range or
   * before another one. */
  assert(range != free_ranges_end);
  assert(range->end > view_base || next != free_ranges_end);

  /* Free ranges addresses are aligned at granularity_mask while the views may
   * be not. */
  ...
  TRACE_(virtual_ranges)("%p - %p, aligned %p - %p.\n", view->base,
                         (char *)view->base + view->size, view_base, view_end);
  ...
  if (range->base < view_base && range->end > view_end) {
    memmove(next + 1, next,
            (free_ranges_end - next) * sizeof(struct range_entry));
    free_ranges_end += 1;
...
static void free_ranges_remove_view(struct file_view *view) {
  ...
  /* Free ranges addresses are aligned at granularity_mask while the views may
   * be not. */
  struct file_view *prev_view =
      RB_ENTRY_VALUE(rb_prev(&view->entry), struct file_view, entry);
  struct file_view *next_view =
      RB_ENTRY_VALUE(rb_next(&view->entry), struct file_view, entry);
  void *prev_view_base =
      prev_view ? ROUND_ADDR(prev_view->base, granularity_mask) : NULL;
  void *prev_view_end = prev_view
                            ? ROUND_ADDR((char *)prev_view->base +
                                             prev_view->size + granularity_mask,
                                         granularity_mask)
                            : NULL;
  void *next_view_base =
      next_view ? ROUND_ADDR(next_view->base, granularity_mask) : NULL;
  void *next_view_end = next_view
                            ? ROUND_ADDR((char *)next_view->base +
                                             next_view->size + granularity_mask,
                                         granularity_mask)
                            : NULL;
  ...
  if (range->end == view_base && next->base == view_end) {
    range->end = next->end;
    memmove(next, next + 1,
            (free_ranges_end - next - 1) * sizeof(struct range_entry));
    free_ranges_end -= 1;
    assert(free_ranges_end - free_ranges > 0);
  }
  ...
  /* otherwise create a new one */
    ...
    memmove(range + 1, range,
            (free_ranges_end - range) * sizeof(struct range_entry));
    free_ranges_end += 1;
    if ((char *)free_ranges_end - (char *)free_ranges > view_block_size)
      ERR("Free range sequence is full, trouble ahead!\n");
    assert((char *)free_ranges_end - (char *)free_ranges <= view_block_size);
...
#ifdef _WIN64
  if ((idx >> pages_vprot_shift) >= pages_vprot_size)
    return 0;
...
static SIZE_T get_vprot_range_size(char *base, SIZE_T size, BYTE mask,
                                   BYTE *vprot) {
  ...
  if (aligned_start_idx > end_idx)
    aligned_start_idx = end_idx;

#ifdef _WIN64
  vprot_ptr = pages_vprot[curr_idx >> pages_vprot_shift] +
              (curr_idx & pages_vprot_mask);
#else
...
static void set_page_vprot(const void *addr, size_t size, BYTE vprot) {
  ...
  while (idx >> pages_vprot_shift != end >> pages_vprot_shift) {
    ...
    memset(pages_vprot[idx >> pages_vprot_shift] + (idx & pages_vprot_mask),
           vprot, dir_size);
    idx += dir_size;
  }
  memset(pages_vprot[idx >> pages_vprot_shift] + (idx & pages_vprot_mask),
 ...
 */
static void set_page_vprot_bits(const void *addr, size_t size, BYTE set,
                                BYTE clear) {
  size_t idx = (size_t)addr >> page_shift;
  size_t end = ((size_t)addr + size + page_mask) >> page_shift;
  ...
  for (; idx < end; idx++) {
    BYTE *ptr =
        pages_vprot[idx >> pages_vprot_shift] + (idx & pages_vprot_mask);
    *ptr = (*ptr & ~clear) | set;
  }
#else
  for (; idx < end; idx++)
    pages_vprot[idx] = (pages_vprot[idx] & ~clear) | set;
#endif
}
...
static BOOL alloc_pages_vprot(const void *addr, size_t size) {
  ...
  for (i = idx >> pages_vprot_shift;
       i < (end + pages_vprot_mask) >> pages_vprot_shift; i++) {
    if (pages_vprot[i])
      continue;
    if ((ptr = anon_mmap_alloc(pages_vprot_mask + 1, PROT_READ | PROT_WRITE)) ==
        MAP_FAILED) {
...
static inline UINT64 maskbits(size_t idx) { return ~(UINT64)0 << (idx & 63); }
...
static void set_arm64ec_range(const void *addr, size_t size) {
  ...
  if (end_pos > pos) {
    ...
    while (pos < end_pos)
      map[pos++] = ~(UINT64)0;
    if (end & 63)
      map[pos] |= ~maskbits(end);
  } else
    map[pos] |= maskbits(idx) & ~maskbits(end);
}
...
static const char *get_prot_str(BYTE prot) {
  ...
  buffer[1] =
      (prot & VPROT_GUARD) ? 'g' : ((prot & VPROT_WRITEWATCH) ? 'H' : '-');
  buffer[2] = (prot & VPROT_READ) ? 'r' : '-';
  buffer[3] =
      (prot & VPROT_WRITECOPY) ? 'W' : ((prot & VPROT_WRITE) ? 'w' : '-');
 ...
 *           find_view
 *
 * Find the view containing a given address. virtual_mutex must be held by
 * caller.
 *
 * PARAMS
 *      addr  [I] Address
...
static struct file_view *find_view(const void *addr, size_t size) {
  ...
  if ((const char *)addr + size < (const char *)addr)
  ...
  while (ptr) {
    ...
    if (view->base > addr)
      ptr = ptr->left;
    else if ((const char *)view->base + view->size <= (const char *)addr)
      ptr = ptr->right;
    else if ((const char *)view->base + view->size < (const char *)addr + size)
...
static struct file_view *find_view_range(const void *addr, size_t size) {
  ...
  while (ptr) {
    ...
    if ((const char *)view->base >= (const char *)addr + size)
      ptr = ptr->left;
    else if ((const char *)view->base + view->size <= (const char *)addr)
      ptr = ptr->right;
    else
 ...
 * virtual_mutex must be held by caller.
 */
static struct wine_rb_entry *
find_view_inside_range(void **base_ptr, void **end_ptr, int top_down) {
...
static void *try_map_free_area(void *base, void *end, ptrdiff_t step,
  ...
  while (start && base <= start && (char *)start + size <= (char *)end) {
    if (anon_mmap_tryfixed(start, size, unix_prot, 0) != MAP_FAILED)
      return start;
    TRACE("Found free area is already mapped, start %p.\n", start);
    if (errno != EEXIST) {
      ERR("mmap() error %s, range %p-%p, unix_prot %#x.\n", strerror(errno),
          start, (char *)start + size, unix_prot);
      return NULL;
    }
    if ((step > 0 && (char *)end - (char *)start < step) ||
        (step < 0 && (char *)start - (char *)base < -step) || step == 0)
      break;
 ...
 * virtual_mutex must be held by caller.
 */
static void *map_free_area(void *base, void *end, size_t size, int top_down,
  ...
  if (top_down) {
    ...
    while (first) {
      struct file_view *view =
          WINE_RB_ENTRY_VALUE(first, struct file_view, entry);
      if ((start = try_map_free_area((char *)view->base + view->size,
                                     (char *)start + size, step, start, size,
                                     unix_prot)))
        break;
      start = ROUND_ADDR((char *)view->base - size, align_mask);
      /* stop if remaining space is not large enough */
      if (!start || start >= end || start < base)
        return NULL;
      first = rb_prev(first);
    }
  } else {
    ...
    while (first) {
      struct file_view *view =
          WINE_RB_ENTRY_VALUE(first, struct file_view, entry);
      if ((start = try_map_free_area(start, view->base, step, start, size,
                                     unix_prot)))
        break;
      start =
          ROUND_ADDR((char *)view->base + view->size + align_mask, align_mask);
      /* stop if remaining space is not large enough */
      if (!start || start >= end || (char *)end - (char *)start < size)
        return NULL;
      first = rb_next(first);
 ...
 */
static void *find_reserved_free_area(void *base, void *end, size_t size,
  ...
  if (top_down) {
    ...
    do {
      if (start >= end || start < base || (char *)end - (char *)start < size)
        return NULL;
      if (start < range->end && start >= range->base &&
          (char *)range->end - (char *)start >= size)
        break;
      if (--range < free_ranges)
        return NULL;
      start = ROUND_ADDR((char *)range->end - size, align_mask);
    } while (1);
  } else {
...
static void remove_reserved_area(void *addr, size_t size) {
  ...
  WINE_RB_FOR_EACH_ENTRY(view, &views_tree, struct file_view, entry) {
    if ((char *)view->base >= (char *)addr + size)
      break;
    if ((char *)view->base + view->size <= (char *)addr)
      continue;
    if (view->base > addr)
...
static void unmap_area(void *start, size_t size) {
  ...
  if (!(size = unmap_area_above_user_limit(start, size)))
    return;
  ...
  LIST_FOR_EACH_ENTRY(area, &reserved_areas, struct reserved_area, entry) {
    ...
    if (area_end >= end) {
      anon_mmap_fixed(start, (char *)end - (char *)start, PROT_NONE,
                      MAP_NORESERVE);
      return;
    }
    anon_mmap_fixed(start, (char *)area_end - (char *)start, PROT_NONE,
                    MAP_NORESERVE);
    start = area_end;
  }
...
static struct file_view *alloc_view(void) {
  ...
  if (view_block_start == view_block_end) {
    ...
    view_block_end =
        view_block_start + view_block_size / sizeof(*view_block_start);
  }
 ...
 *           unregister_view
 *
 * Remove view from the tree and update free ranges. virtual_mutex must be held
 * by caller.
 */
...
static void delete_view(struct file_view *view) /* [in] View */
{
  ...
  if (view->protect & VPROT_ARM64EC)
    clear_arm64ec_range(view->base, view->size);
 ...
 *           register_view
 *
 * Add view to the tree and update free ranges. virtual_mutex must be held by
 * caller.
 */
 ...
 */
static NTSTATUS create_view(struct file_view **view_ret, void *base,
                            size_t size, unsigned int vprot) {
  struct file_view *view;
  ...
  while ((view = find_view_range(base, size))) {
    TRACE("overlapping view %p-%p for %p-%p\n", view->base,
          (char *)view->base + view->size, base, (char *)base + size);
...
static inline int mprotect_exec(void *base, size_t size, int unix_prot) {
  if (force_exec_prot && (unix_prot & PROT_READ) && !(unix_prot & PROT_EXEC)) {
    TRACE("forcing exec permission on %p-%p\n", base, (char *)base + size - 1);
    if (!mprotect(base, size, unix_prot | PROT_EXEC))
      return 0;
 ...
 *           mprotect_range
 *
 * Call mprotect on a page range, applying the protections from the per-page
 * byte.
 */
static void mprotect_range(void *base, size_t size, BYTE set, BYTE clear) {
  ...
  for (count = i = 1; i < size >> page_shift; i++, count++) {
    next = get_unix_prot(
        (get_page_vprot(addr + (count << page_shift)) & ~clear) | set);
    if (next == prot)
      continue;
 ...
 */
static BOOL set_vprot(struct file_view *view, void *base, size_t size,
                      BYTE vprot) {
  if (view->protect & VPROT_WRITEWATCH) {
    /* each page may need different protections depending on write watch flag */
    set_page_vprot_bits(base, size, vprot & ~VPROT_WRITEWATCH,
                        ~vprot & ~VPROT_WRITEWATCH);
    mprotect_range(base, size, 0, 0);
    return TRUE;
  }
  if (enable_write_exceptions && is_vprot_exec_write(vprot))
    vprot |= VPROT_WRITEWATCH;
  if (mprotect_exec(base, size, get_unix_prot(vprot)))
    return FALSE;
  set_page_vprot(base, size, vprot);
  return TRUE;
}
...
static NTSTATUS set_protection(struct file_view *view, void *base, SIZE_T size,
  ...
  if ((status = get_vprot_flags(protect, &vprot, view->protect & SEC_IMAGE)))
    return status;
...
static void commit_arm64ec_map(struct file_view *view) {
  ...
  set_vprot(arm64ec_view, base, size,
 ...
 */
static void update_write_watches(void *base, size_t size,
                                 size_t accessed_size) {
  TRACE("updating watch %p-%p-%p\n", base, (char *)base + accessed_size,
        (char *)base + size);
 ...
 *           unmap_extra_space
 *
 * Release the extra memory while keeping the range starting on the alignment
 * boundary.
 */
static inline void *unmap_extra_space(void *ptr, size_t total_size,
                                      size_t wanted_size, size_t align_mask) {
 ...
 *
 * Find a free area inside a reserved area, skipping the preloader reserved
 * range. virtual_mutex must be held by caller.
 */
static void *find_reserved_free_area_outside_preloader(void *start, void *end,
  ...
  if (preload_reserve_end >= end) {
    if (preload_reserve_start <= start)
      return NULL; /* no space in that area */
    if (preload_reserve_start < end)
  ...
  {
    if (top_down) {
      ret = find_reserved_free_area(preload_reserve_end, end, size, top_down,
                                    align_mask);
      if (ret)
        return ret;
      end = preload_reserve_start;
    } else {
      ret = find_reserved_free_area(start, preload_reserve_start, size,
                                    top_down, align_mask);
      if (ret)
        return ret;
      start = preload_reserve_end;
...
static void *map_reserved_area(void *limit_low, void *limit_high, size_t size,
                               int top_down, int unix_prot, size_t align_mask) {
  void *ptr = NULL;
  struct reserved_area *area;

  if (top_down) {
    LIST_FOR_EACH_ENTRY_REV(area, &reserved_areas, struct reserved_area,
                            entry) {
      ...
      ptr = find_reserved_free_area_outside_preloader(start, end, size,
                                                      top_down, align_mask);
      if (ptr)
        break;
    }
...
static NTSTATUS map_fixed_area(void *base, size_t size, unsigned int vprot) {
  ...
  LIST_FOR_EACH_ENTRY(area, &reserved_areas, struct reserved_area, entry) {
    ...
    if (area_end <= start)
      continue;
    if (area_start > start) {
      if (anon_mmap_tryfixed(start, area_start - start, unix_prot, 0) ==
          MAP_FAILED)
        goto failed;
      start = area_start;
    }
    if (area_end >= end) {
      if (anon_mmap_fixed(start, end - start, unix_prot, 0) == MAP_FAILED)
        goto failed;
      return STATUS_SUCCESS;
    }
    if (anon_mmap_fixed(start, area_end - start, unix_prot, 0) == MAP_FAILED)
      goto failed;
    start = area_end;
  }

  if (anon_mmap_tryfixed(start, end - start, unix_prot, 0) == MAP_FAILED)
    goto failed;
  return STATUS_SUCCESS;
...
static NTSTATUS map_view(struct file_view **view_ret, void *base, size_t size,
  ...
  if (alloc_type & MEM_REPLACE_PLACEHOLDER) {
    ...
    if (view->base != base || view->size != size)
      return STATUS_CONFLICTING_ADDRESSES;
    if (!(view->protect & VPROT_FREE_PLACEHOLDER))
      return STATUS_INVALID_PARAMETER;

    TRACE("found view %p, size %p, protect %#x.\n", view->base,
          (void *)view->size, view->protect);
  ...
  if (limit_high && limit_low >= limit_high)
    return STATUS_INVALID_PARAMETER;

  if (base) {
    if (is_beyond_limit(base, size, address_space_limit))
      return STATUS_WORKING_SET_LIMIT_RANGE;
    if (limit_low && base < (void *)limit_low)
      return STATUS_CONFLICTING_ADDRESSES;
    if (limit_high && is_beyond_limit(base, size, (void *)limit_high))
      return STATUS_CONFLICTING_ADDRESSES;
    if (is_beyond_limit(base, size, host_addr_space_limit))
      return STATUS_CONFLICTING_ADDRESSES;
    if ((status = map_fixed_area(base, size, vprot)))
      return status;
    if (is_beyond_limit(base, size, working_set_limit))
      working_set_limit = address_space_limit;
    ptr = base;
  } else {
    ...
    if (!align_mask)
      align_mask = granularity_mask;
    view_size = size + align_mask + 1;

    if (limit_low && (void *)limit_low > start)
      start = (void *)limit_low;
    if (limit_high && (void *)limit_high < end)
      end = (char *)limit_high + 1;

    if ((ptr = map_reserved_area(start, end, size, top_down,
                                 get_unix_prot(vprot), align_mask))) {
      TRACE("got mem in reserved area %p-%p\n", ptr, (char *)ptr + size);
      goto done;
    }

    if (start > address_space_start || end < host_addr_space_limit ||
        top_down) {
      if (!(ptr = map_free_area(start, end, size, top_down,
                                get_unix_prot(vprot), align_mask)))
        return STATUS_NO_MEMORY;
    ...
    for (;;) {
      if ((ptr = anon_mmap_alloc(view_size, get_unix_prot(vprot))) ==
          MAP_FAILED) {
        status =
            (errno == ENOMEM) ? STATUS_NO_MEMORY : STATUS_INVALID_PARAMETER;
        ERR("anon mmap error %s, size %p, unix_prot %#x\n", strerror(errno),
            (void *)view_size, get_unix_prot(vprot));
        return status;
      }
 ...
 *           map_file_into_view
 *
 * Wrapper for mmap() to map a file into a view, falling back to read if mmap
 * fails. virtual_mutex must be held by caller.
 */
static NTSTATUS map_file_into_view(struct file_view *view, int fd, size_t start,
                                   size_t size, off_t offset,
                                   unsigned int vprot, BOOL removable) {
  void *ptr;
  int prot =
      get_unix_prot(vprot | VPROT_COMMITTED /* make sure it is accessible */);
  unsigned int flags =
      MAP_FIXED | ((vprot & VPROT_WRITECOPY) ? MAP_PRIVATE : MAP_SHARED);
  ...
  if (!removable || (flags & MAP_SHARED)) {
    if (mmap((char *)view->base + start, size, prot, flags, fd, offset) !=
        MAP_FAILED)
      goto done;
    ...
    case EPERM: /* noexec filesystem, fall back to read() */
      if (prot & PROT_EXEC) {
        if (flags & MAP_SHARED)
          WARN("failed to set PROT_EXEC on shared file map, noexec filesystem? "
    ...
    default:
      ERR("mmap error %s, range %p-%p, unix_prot %#x\n", strerror(errno),
          (char *)view->base + start, (char *)view->base + start + size, prot);
      return STATUS_NO_MEMORY;
    }
  }

  /* Reserve the memory with an anonymous mmap */
  ptr = anon_mmap_fixed((char *)view->base + start, size,
 ...
 *           get_committed_size
 *
 * Get the size of the committed range with equal masked vprot bytes starting at
 * base. Also return the protections for the first page.
 */
static SIZE_T get_committed_size(struct file_view *view, void *base,
                                 size_t max_size, BYTE *vprot,
                                 BYTE vprot_mask) {
  SIZE_T offset, size;
  ...
  if (view->protect & SEC_RESERVE) {
    ...
    *vprot = get_page_vprot(base);
    ...
    if (!size || !(vprot_mask & ~VPROT_COMMITTED))
      return size;
  } else
    size = min(view->size - offset, max_size);
 ...
 * virtual_mutex must be held by caller.
 */
static NTSTATUS decommit_pages(struct file_view *view, size_t start,
                               size_t size) {
  if (!size)
    size = view->size;
  if (anon_mmap_fixed((char *)view->base + start, size, PROT_NONE, 0) !=
      MAP_FAILED) {
 ...
 * virtual_mutex must be held by caller.
 */
static NTSTATUS remove_pages_from_view(struct file_view *view, char *base,
                                       size_t size) {
 ...
 * virtual_mutex must be held by caller.
 */
static NTSTATUS free_pages_preserve_placeholder(struct file_view *view,
                                                char *base, size_t size) {
  NTSTATUS status;

  if (!size)
    return STATUS_INVALID_PARAMETER_3;
  if (!(view->protect & VPROT_PLACEHOLDER))
    return STATUS_CONFLICTING_ADDRESSES;
  if (view->protect & VPROT_FREE_PLACEHOLDER && size == view->size)
    return STATUS_CONFLICTING_ADDRESSES;
  ...
  if (size < view->size) {
    ...
    status = create_view(&view, base, size,
                         VPROT_PLACEHOLDER | VPROT_FREE_PLACEHOLDER);
    if (status)
      return status;
  }
 ...
 * virtual_mutex must be held by caller.
 */
static NTSTATUS coalesce_placeholders(struct file_view *view, char *base,
                                      size_t size) {
  ...
  while (curr_view->protect & VPROT_FREE_PLACEHOLDER) {
    ...
    if (views_size >= size)
      break;
    if (!(next = rb_next(&curr_view->entry)))
      break;
    next_view = RB_ENTRY_VALUE(next, struct file_view, entry);
    if ((char *)curr_view->base + curr_view->size != next_view->base)
      break;
    curr_view = next_view;
  }

  if (view_count < 2 || size != views_size)
    return STATUS_CONFLICTING_ADDRESSES;
 ...
 */
static NTSTATUS allocate_dos_memory(struct file_view **view,
                                    unsigned int vprot) {
  size_t size;
  void *addr = NULL;
  ...
  if (find_view_range(0, dosmem_size))
    return STATUS_CONFLICTING_ADDRESSES;
  ...
  if (mmap_is_in_reserved_area(NULL, 0x10000) != 1) {
    addr = anon_mmap_tryfixed((void *)page_size, 0x10000 - page_size, unix_prot,
                              0);
    if (addr != MAP_FAILED) {
...
static NTSTATUS map_pe_header(void *ptr, size_t size, int fd, BOOL *removable) {
  ...
  if (!*removable) {
    if (mmap(ptr, size, PROT_READ | PROT_WRITE | PROT_EXEC,
...
static void alloc_arm64ec_map(void) {
  unsigned int status;
  SIZE_T size = ((ULONG_PTR)address_space_limit + page_size) >>
                (page_shift + 3); /* one bit per page */

  size = ROUND_SIZE(0, size);
  status = map_view(&arm64ec_view, NULL, size, MEM_TOP_DOWN,
                    VPROT_READ | VPROT_COMMITTED, 0, 0, granularity_mask);
  if (status) {
...
static void update_arm64ec_ranges(struct file_view *view, IMAGE_NT_HEADERS *nt,
  ...
  metadata = (void *)(base + (cfg->CHPEMetadataPointer -
                              nt->OptionalHeader.ImageBase));
  *entry_point = redirect_arm64ec_rva(
      base, nt->OptionalHeader.AddressOfEntryPoint, metadata);
  if (!metadata->CodeMap)
    return;
  map = (void *)(base + metadata->CodeMap);

  for (i = 0; i < metadata->CodeMapCount; i++) {
    if ((map[i].StartOffset & 0x3) != 1 /* arm64ec */)
      continue;
 ...
 */
static void apply_arm64x_relocations(char *base,
                                     const IMAGE_BASE_RELOCATION *reloc,
                                     size_t size) {
  const IMAGE_BASE_RELOCATION *reloc_end =
      (const IMAGE_BASE_RELOCATION *)((const char *)reloc + size);
...
static void update_arm64x_mapping(struct file_view *view, IMAGE_NT_HEADERS *nt,
  ...
  if (size <=
      offsetof(IMAGE_LOAD_CONFIG_DIRECTORY, DynamicValueRelocTableSection))
    return;
  offset = cfg->DynamicValueRelocTableOffset;
  sec = cfg->DynamicValueRelocTableSection;
  if (!sec || sec > nt->FileHeader.NumberOfSections)
    return;
  if (offset >= sections[sec - 1].Misc.VirtualSize)
    return;
  table = (const IMAGE_DYNAMIC_RELOCATION_TABLE
               *)(base + sections[sec - 1].VirtualAddress + offset);
  ptr = (const char *)(table + 1);
  end = ptr + table->Size;
  switch (table->Version) {
  case 1:
    while (ptr < end) {
      const IMAGE_DYNAMIC_RELOCATION64 *dyn =
          (const IMAGE_DYNAMIC_RELOCATION64 *)ptr;
  ...
  case 2:
    while (ptr < end) {
      const IMAGE_DYNAMIC_RELOCATION64_V2 *dyn =
          (const IMAGE_DYNAMIC_RELOCATION64_V2 *)ptr;
      if (dyn->Symbol == IMAGE_DYNAMIC_RELOCATION_ARM64X) {
        apply_arm64x_relocations(base, (const IMAGE_BASE_RELOCATION *)(dyn + 1),
                                 dyn->FixupInfoSize);
        break;
 ...
 *           get_data_dir
 */
static IMAGE_DATA_DIRECTORY *get_data_dir(IMAGE_NT_HEADERS *nt,
                                          SIZE_T total_size, ULONG dir) {
  IMAGE_DATA_DIRECTORY *data;

  switch (nt->OptionalHeader.Magic) {
  case IMAGE_NT_OPTIONAL_HDR64_MAGIC:
    if (dir >= ((IMAGE_NT_HEADERS64 *)nt)->OptionalHeader.NumberOfRvaAndSizes)
      return NULL;
    data = &((IMAGE_NT_HEADERS64 *)nt)->OptionalHeader.DataDirectory[dir];
    break;
  case IMAGE_NT_OPTIONAL_HDR32_MAGIC:
    if (dir >= ((IMAGE_NT_HEADERS32 *)nt)->OptionalHeader.NumberOfRvaAndSizes)
      return NULL;
  ...
  if (!data->VirtualAddress)
    return NULL;
  if (data->VirtualAddress >= total_size)
    return NULL;
  if (data->Size > total_size - data->VirtualAddress)
    return NULL;
  return data;
}
...
process_relocation_block(char *page, IMAGE_BASE_RELOCATION *rel,
                         INT_PTR delta) {
  USHORT *reloc = (USHORT *)(rel + 1);
  unsigned int count;

  for (count = (rel->SizeOfBlock - sizeof(*rel)) / sizeof(USHORT); count;
       count--, reloc++) {
    ...
    case IMAGE_REL_BASED_THUMB_MOV32: {
      ...
      lo = LOWORD(imm);
      hi = HIWORD(imm);
      inst[0] = (inst[0] & 0x8f00fbf0) + ((lo >> 1) & 0x0400) +
                ((lo >> 12) & 0x000f) + ((lo << 20) & 0x70000000) +
                ((lo << 16) & 0xff0000);
      inst[1] = (inst[1] & 0x8f00fbf0) + ((hi >> 1) & 0x0400) +
                ((hi >> 12) & 0x000f) + ((hi << 20) & 0x70000000) +
                ((hi << 16) & 0xff0000);
      break;
    }
    default:
 ...
 * virtual_mutex must be held by caller.
 */
static NTSTATUS map_image_into_view(struct file_view *view,
                                    const WCHAR *filename, int fd,
                                    struct pe_image_info *image_info,
                                    USHORT machine, int shared_fd,
                                    BOOL removable) {
  ...
  if ((status = map_pe_header(view->base, header_size, fd, &removable)))
    return status;
  ...
  if ((char *)(nt + 1) > header_end)
    return status;
  if (nt->FileHeader.NumberOfSections > ARRAY_SIZE(sections))
    return status;
  sec = IMAGE_FIRST_SECTION(nt);
  if ((char *)(sec + nt->FileHeader.NumberOfSections) > header_end)
    return status;
  /* Some applications (e.g. the Steam version of Borderlands) map over the top
   * of the section headers, copying the headers into local memory is necessary
   * to properly load such applications. */
  ...
  if (image_info->image_flags & IMAGE_FLAGS_ImageMappedFlat) {
    ...
    if (map_file_into_view(view, fd, 0, total_size, 0,
  ...
  for (i = pos = 0; i < nt->FileHeader.NumberOfSections; i++, sec++) {
    ...
    /* file positions are rounded to sector boundaries regardless of
    ...
    if (sec->VirtualAddress > total_size || end > total_size ||
        end < sec->VirtualAddress) {
      WARN_(module)("%s section %.8s too large (%x+%lx/%lx)\n",
                    debugstr_w(filename), sec->Name, (int)sec->VirtualAddress,
                    map_size, total_size);
      return status;
    }
    ...
    if ((sec->Characteristics & IMAGE_SCN_MEM_SHARED) &&
      ...
      TRACE_(module)("%s mapping shared section %.8s at %p off %x (%x) size "
                     "%lx (%lx) flags %x\n",
      ...
      if (map_file_into_view(view, shared_fd, sec->VirtualAddress, map_size,
                             pos, VPROT_COMMITTED | VPROT_READ | VPROT_WRITE,
      ...
      if (imports && imports->VirtualAddress >= sec->VirtualAddress &&
        ...
        UINT_PTR end =
            base + ROUND_SIZE(imports->VirtualAddress, imports->Size);
        if (end > sec->VirtualAddress + map_size)
          end = sec->VirtualAddress + map_size;
        if (end > base)
    ...
    if (!sec->PointerToRawData || !file_size)
      continue;

    /* Note: if the section is not aligned properly map_file_into_view will
     * magically fall back to read(), so we don't need to check anything here.
     */
  ...
  if (image_info->map_addr &&
      (delta = image_info->map_addr - image_info->base)) {
    TRACE_(module)("relocating %s dynamic base %lx -> %lx mapped at %p\n",
                   debugstr_w(filename), (ULONG_PTR)image_info->base,
                   (ULONG_PTR)image_info->map_addr, ptr);
    ...
    if ((dir = get_data_dir(nt, total_size, IMAGE_DIRECTORY_ENTRY_BASERELOC))) {
      IMAGE_BASE_RELOCATION *rel =
          (IMAGE_BASE_RELOCATION *)(ptr + dir->VirtualAddress);
      IMAGE_BASE_RELOCATION *end =
          (IMAGE_BASE_RELOCATION *)((char *)rel + dir->Size);

      while (rel && rel < end - 1 && rel->SizeOfBlock &&
             rel->VirtualAddress < total_size)
  ...
  set_vprot(view, ptr, ROUND_SIZE(0, header_size),
  ...
  for (i = 0; i < nt->FileHeader.NumberOfSections; i++, sec++) {
    ...
    if (!set_vprot(view, ptr + sec->VirtualAddress, size, vprot) &&
        (vprot & VPROT_EXEC))
      ERR("failed to set %08x protection on %s section %.8s, noexec "
          "filesystem?\n",
  ...
  VALGRIND_LOAD_PDB_DEBUGINFO(
      fd, ptr, total_size, ptr - (char *)wine_server_get_ptr(image_info->base));
#endif
  return STATUS_SUCCESS;
}
 ...
 *             get_mapping_info
 */
static unsigned int get_mapping_info(HANDLE handle, ACCESS_MASK access,
  ...
  for (;;) {
    ...
    SERVER_END_REQ;
    if (!status && total <= size - sizeof(WCHAR))
      break;
    free(image_info);
    if (status)
      return status;
 ...
 */
static NTSTATUS map_image_view(struct file_view **view_ret,
                               struct pe_image_info *image_info, SIZE_T size,
                               ULONG_PTR limit_low, ULONG_PTR limit_high,
                               ULONG alloc_type) {
  unsigned int vprot = SEC_IMAGE | SEC_FILE | VPROT_COMMITTED | VPROT_READ |
                       VPROT_EXEC | VPROT_WRITECOPY;
  void *base;
  NTSTATUS status;
  ULONG_PTR start, end;
  BOOL top_down =
  ...
  limit_low = max(
      limit_low,
      (ULONG_PTR)address_space_start); /* make sure the DOS area remains free */
  if (!limit_high)
    limit_high = (ULONG_PTR)user_space_limit;

  /* first try the specified base */

  if (image_info->map_addr) {
    base = wine_server_get_ptr(image_info->map_addr);
    if ((ULONG_PTR)base != image_info->map_addr)
      base = NULL;
  } else {
    base = wine_server_get_ptr(image_info->base);
    if ((ULONG_PTR)base != image_info->base)
      base = NULL;
  }
  if (base) {
    status = map_view(view_ret, base, size, alloc_type, vprot, limit_low,
                      limit_high, 0);
    if (!status)
      return status;
  }
  ...
  if (start < end && (start != limit_low || end != limit_high)) {
    status = map_view(view_ret, NULL, size, top_down ? MEM_TOP_DOWN : 0, vprot,
                      start, end, 0);
    if (!status)
      return status;
  }
  ...
  return map_view(view_ret, NULL, size, top_down ? MEM_TOP_DOWN : 0, vprot,
 ...
 */
static NTSTATUS virtual_map_image(HANDLE mapping, void **addr_ptr,
                                  SIZE_T *size_ptr, HANDLE shared_file,
                                  ULONG_PTR limit_low, ULONG_PTR limit_high,
                                  ULONG alloc_type, USHORT machine,
                                  struct pe_image_info *image_info,
  ...
  if ((status =
           server_get_unix_fd(mapping, 0, &unix_fd, &needs_close, NULL, NULL)))
    return status;

  if (shared_file && ((status = server_get_unix_fd(
                           shared_file, FILE_READ_DATA | FILE_WRITE_DATA,
  ...
  if (!image_info->map_addr && (image_info->image_charact & IMAGE_FILE_DLL) &&
      (image_info->image_flags & IMAGE_FLAGS_ImageDynamicallyRelocated)) {
    SERVER_START_REQ(get_image_map_address) {
      req->handle = wine_server_obj_handle(mapping);
      if (!wine_server_call(req))
        image_info->map_addr = reply->addr;
    }
    SERVER_END_REQ;
  }
  ...
  status = map_image_view(&view, image_info, size, limit_low, limit_high,
                          alloc_type);
  if (status)
    goto done;

  status = map_image_into_view(view, filename, unix_fd, image_info, machine,
                               shared_fd, needs_close);
  if (status == STATUS_SUCCESS) {
  ...
  if (NT_SUCCESS(status)) {
    if (is_builtin)
      add_builtin_module(view->base, NULL);
    *addr_ptr = view->base;
    *size_ptr = size;
    VIRTUAL_DEBUG_DUMP_VIEW(view);
  } else
 ...
 */
static unsigned int
virtual_map_section(HANDLE handle, PVOID *addr_ptr, ULONG_PTR limit_low,
  ...
  if (image_info) {
    ...
    if (NtCurrentTeb64()) {
      ...
      NtCurrentTeb64()->Tib.ArbitraryUserPointer =
          PtrToUlong(NtCurrentTeb()->Tib.ArbitraryUserPointer);
    }
    filename = (WCHAR *)(image_info + 1);
    /* check if we can replace that mapping with the builtin */
    res = load_builtin(image_info, filename, machine, &info, addr_ptr, size_ptr,
                       limit_low, limit_high);
    if (res == STATUS_IMAGE_ALREADY_LOADED)
      res = virtual_map_image(handle, addr_ptr, size_ptr, shared_file,
                              limit_low, limit_high, alloc_type, machine,
                              image_info, filename, FALSE);
    if (shared_file)
      NtClose(shared_file);
    free(image_info);
    if (NtCurrentTeb64())
  ...
  if (!(size = ROUND_SIZE(0, size)))
    return STATUS_INVALID_PARAMETER; /* wrap-around */
  ...
  res =
      map_view(&view, base, size, alloc_type, vprot, limit_low, limit_high, 0);
  if (res)
    goto done;
...
static void *alloc_virtual_heap(SIZE_T size) {
  ...
  LIST_FOR_EACH_ENTRY_REV(area, &reserved_areas, struct reserved_area, entry) {
    ...
    if (is_win64 && base < (void *)0x80000000)
      break;
    if (preload_reserve_end >= end) {
      if (preload_reserve_start <= base)
        continue; /* no space in that area */
      if (preload_reserve_start < end)
        end = preload_reserve_start;
    } else if (preload_reserve_end > base) {
      if (preload_reserve_start <= base)
        base = preload_reserve_end;
      else if ((char *)end - (char *)preload_reserve_end >= size)
        base = preload_reserve_end;
      else
        end = preload_reserve_start;
    }
    if ((char *)end - (char *)base < size)
      continue;
...
void virtual_init(void) {
  const struct preload_info **preload_info =
      dlsym(RTLD_DEFAULT, "wine_main_preload_info");
  const char *preload;
  size_t size;
  int i;
  ...
  /* try to find space in a reserved area for the views and pages protection
   * table */
#ifdef _WIN64
  pages_vprot_size =
      ((size_t)host_addr_space_limit >> page_shift >> pages_vprot_shift) + 1;
  ...
  /* make the DOS area accessible (except the low 64K) to hide bugs in broken
   * apps like Excel 2003 */
...
void virtual_get_system_info(SYSTEM_BASIC_INFORMATION *info, BOOL wow64) {
#if defined(HAVE_SYSINFO) && defined(HAVE_STRUCT_SYSINFO_TOTALRAM) &&          \
    defined(HAVE_STRUCT_SYSINFO_MEM_UNIT)
  ...
  info->MmNumberOfPhysicalPages =
      info->MmHighestPhysicalPage - info->MmLowestPhysicalPage;
...
NTSTATUS virtual_map_builtin_module(HANDLE mapping, void **module, SIZE_T *size,
  ...
  if ((status = get_mapping_info(mapping, SECTION_MAP_READ, &sec_flags,
                                 &full_size, &shared_file, &image_info)))
    return status;
  ...
  *size = 0;
  ...
  if (!image_info->wine_builtin) /* ignore non-builtins */
  {
  ...
  } else if (prefer_native && (image_info->dll_charact &
    ...
    status =
        virtual_map_image(mapping, module, size, shared_file, limit_low,
                          limit_high, 0, machine, image_info, filename, TRUE);
 ...
 *           virtual_map_module
 */
NTSTATUS virtual_map_module(HANDLE mapping, void **module, SIZE_T *size,
                            SECTION_IMAGE_INFORMATION *info,
                            ULONG_PTR limit_low, ULONG_PTR limit_high,
                            USHORT machine) {
  unsigned int status;
  ...
  *size = 0;
  ...
  /* check if we can replace that mapping with the builtin */
  status = load_builtin(image_info, filename, machine, info, module, size,
                        limit_low, limit_high);
  if (status == STATUS_IMAGE_ALREADY_LOADED) {
    status =
        virtual_map_image(mapping, module, size, shared_file, limit_low,
                          limit_high, 0, machine, image_info, filename, FALSE);
    virtual_fill_image_information(image_info, info);
  }
  if (shared_file)
    NtClose(shared_file);
...
NTSTATUS virtual_create_builtin_view(void *module,
  ...
  server_enter_uninterrupted_section(&virtual_mutex, &sigset);
  status = create_view(&view, base, size,
                       SEC_IMAGE | SEC_FILE | VPROT_SYSTEM | VPROT_COMMITTED |
  ...
  if (!status) {
    ...
    for (i = 0; i < nt->FileHeader.NumberOfSections; i++) {
      ...
      if (sec[i].Characteristics & IMAGE_SCN_MEM_WRITE)
        flags |= VPROT_WRITE;
      set_page_vprot((char *)base + sec[i].VirtualAddress,
                     sec[i].Misc.VirtualSize, flags);
    }
...
NTSTATUS virtual_relocate_module(void *module) {
  char *ptr = module;
  IMAGE_NT_HEADERS *nt =
      (IMAGE_NT_HEADERS *)(ptr + ((IMAGE_DOS_HEADER *)module)->e_lfanew);
  ...
  if (!(delta = (ULONG_PTR)module - image_base))
    return STATUS_SUCCESS;

  if (nt->FileHeader.Characteristics & IMAGE_FILE_RELOCS_STRIPPED) {
    ERR("Need to relocate module from %p to %p, but relocation records are "
        "stripped\n",
  ...
  for (i = 0; i < nt->FileHeader.NumberOfSections; i++) {
    ...
    NtProtectVirtualMemory(NtCurrentProcess(), &addr, &size, PAGE_READWRITE,
                           &protect_old[i]);
  }
  ...
  while (rel && rel < end - 1 && rel->SizeOfBlock &&
         rel->VirtualAddress < total_size)
    rel = process_relocation_block((char *)module + rel->VirtualAddress, rel,
                                   delta);

  for (i = 0; i < nt->FileHeader.NumberOfSections; i++) {
    void *addr = (char *)module + sec[i].VirtualAddress;
    SIZE_T size = sec[i].SizeOfRawData;
    NtProtectVirtualMemory(NtCurrentProcess(), &addr, &size, protect_old[i],
                           &protect_old[i]);
  }
  return STATUS_SUCCESS;
}
...
static TEB *init_teb(void *ptr, BOOL is_wow) {
  ...
  teb32->ActivationContextStackPointer =
      PtrToUlong(&teb32->ActivationContextStack);
  ...
  teb64->ActivationContextStackPointer =
      PtrToUlong(&teb64->ActivationContextStack);
...
TEB *virtual_alloc_first_teb(void) {
  ...
  NtAllocateVirtualMemory(NtCurrentProcess(), (void **)&ptr, 0, &data_size,
                          MEM_COMMIT, PAGE_READWRITE);
  peb =
      (PEB *)((char *)teb_block + 31 * block_size + (is_win64 ? 0 : page_size));
...
void virtual_free_teb(TEB *teb) {
  ...
  if (teb->DeallocationStack) {
    size = 0;
    NtFreeVirtualMemory(GetCurrentProcess(), &teb->DeallocationStack, &size,
                        MEM_RELEASE);
  }
#ifdef __aarch64__
  if (teb->ChpeV2CpuAreaInfo) {
    size = 0;
    NtFreeVirtualMemory(GetCurrentProcess(), (void **)&teb->ChpeV2CpuAreaInfo,
                        &size, MEM_RELEASE);
  }
#endif
  if (thread_data->kernel_stack) {
    size = 0;
    NtFreeVirtualMemory(GetCurrentProcess(), &thread_data->kernel_stack, &size,
                        MEM_RELEASE);
  }
...
NTSTATUS virtual_clear_tls_index(ULONG index) {
  ...
  if (index < TLS_MINIMUM_AVAILABLE) {
    server_enter_uninterrupted_section(&virtual_mutex, &sigset);
    LIST_FOR_EACH_ENTRY(thread_data, &teb_list, struct ntdll_thread_data,
                        entry) {
      TEB *teb = CONTAINING_RECORD(thread_data, TEB, GdiTebBatch);
#ifdef _WIN64
      WOW_TEB *wow_teb = get_wow_teb(teb);
      if (wow_teb)
        wow_teb->TlsSlots[index] = 0;
      else
#endif
    ...
    if (index >= 8 * sizeof(peb->TlsExpansionBitmapBits))
      return STATUS_INVALID_PARAMETER;
 ...
 */
NTSTATUS virtual_alloc_thread_stack(INITIAL_TEB *stack, ULONG_PTR limit_low,
                                    ULONG_PTR limit_high, SIZE_T reserve_size,
  ...
  if (!reserve_size)
    reserve_size = main_image_info.MaximumStackSize;
  if (!commit_size)
    commit_size = main_image_info.CommittedStackSize;

  size = max(reserve_size, commit_size);
  if (size < 1024 * 1024)
    size = 1024 * 1024;             /* Xlib needs a large stack */
  size = (size + 0xffff) & ~0xffff; /* round to 64K boundary */

  server_enter_uninterrupted_section(&virtual_mutex, &sigset);

  status =
      map_view(&view, NULL, size, 0, VPROT_READ | VPROT_WRITE | VPROT_COMMITTED,
               limit_low, limit_high, 0);
  if (status != STATUS_SUCCESS)
    goto done;
...
void virtual_map_user_shared_data(void) {
  static const WCHAR nameW[] = {
      '\\', 'K',  'e', 'r', 'n', 'e', 'l', 'O', 'b', 'j', 'e', 'c', 't',
      's',  '\\', '_', '_', 'w', 'i', 'n', 'e', '_', 'u', 's', 'e', 'r',
      '_',  's',  'h', 'a', 'r', 'e', 'd', '_', 'd', 'a', 't', 'a', 0};
  ...
  if ((res = server_get_unix_fd(section, 0, &fd, &needs_close, NULL, NULL)) ||
      (user_shared_data != mmap(user_shared_data, page_size, PROT_READ,
...
static BOOL is_inside_thread_stack(void *ptr, struct thread_stack_info *stack) {
  ...
  stack->guaranteed =
      max(teb->GuaranteedStackBytes, page_size * (is_win64 ? 2 : 1));
  stack->is_wow = FALSE;
  if ((char *)ptr > stack->start && (char *)ptr <= stack->end)
    return TRUE;
  ...
  stack->guaranteed =
      max(wow_teb->GuaranteedStackBytes, page_size * (is_win64 ? 1 : 2));
  stack->is_wow = TRUE;
...
static NTSTATUS grow_thread_stack(char *page,
  ...
  } else /* inside guaranteed space -> overflow exception */
  {
    ...
    set_page_vprot_bits(page, stack_info->guaranteed, VPROT_COMMITTED,
                        VPROT_GUARD);
...
NTSTATUS virtual_handle_fault(EXCEPTION_RECORD *rec, void *stack) {
  ...
  if (!is_inside_signal_stack(stack) && (vprot & VPROT_GUARD)) {
    ...
    if (vprot & VPROT_WRITEWATCH) {
      if (enable_write_exceptions && is_vprot_exec_write(vprot) &&
          !ntdll_get_thread_data()->allow_writes) {
 ...
 */
void *virtual_setup_exception(void *stack_ptr, size_t size,
  ...
  if (stack < stack_info.start + 4096) {
    ...
    ERR("stack overflow %u bytes addr %p stack %p (%p-%p-%p)\n", diff,
        rec->ExceptionAddress, stack, stack_info.start, stack_info.limit,
        stack_info.end);
    abort_thread(1);
 ...
 *           check_write_access
 *
 * Check if the memory range is writable, temporarily disabling write watches if
 * necessary.
 */
static NTSTATUS check_write_access(void *base, size_t size,
  ...
  if (*has_write_watch)
    mprotect_range(addr, size, 0,
                   VPROT_WRITEWATCH); /* temporarily enable write access */
  return STATUS_SUCCESS;
}
...
unsigned int virtual_locked_server_call(void *req_ptr) {
  ...
  if (!(ret = check_write_access(addr, size, &has_write_watch))) {
    ...
    if (has_write_watch)
      update_write_watches(addr, size, wine_server_reply_size(req));
  } else
...
ssize_t virtual_locked_recvmsg(int fd, struct msghdr *hdr, int flags) {
  ...
  for (i = 0; i < hdr->msg_iovlen; i++)
    if (check_write_access(hdr->msg_iov[i].iov_base, hdr->msg_iov[i].iov_len,
                           &has_write_watch))
      break;
  if (i == hdr->msg_iovlen) {
  ...
  if (has_write_watch)
    while (i--)
      update_write_watches(hdr->msg_iov[i].iov_base, hdr->msg_iov[i].iov_len,
                           0);
...
BOOL virtual_is_valid_code_address(const void *addr, SIZE_T size) {
  ...
  if ((view = find_view(addr, size)))
    ret = !(view->protect &
            VPROT_SYSTEM); /* system views are not visible to the app */
  server_leave_uninterrupted_section(&virtual_mutex, &sigset);
  return ret;
}
 ...
 *
 * Check if a memory buffer can be read, triggering page faults if needed for
 * DIB section access.
 */
 ...
 *
 * Check if a memory buffer can be written to, triggering page faults if needed
 * for write watches.
 */
 ...
 */
SIZE_T virtual_uninterrupted_read_memory(const void *addr, void *buffer,
                                         SIZE_T size) {
  struct file_view *view;
  sigset_t sigset;
  SIZE_T bytes_read = 0;
  ...
  if ((view = find_view(addr, size))) {
    if (!(view->protect & VPROT_SYSTEM)) {
      while (bytes_read < size &&
             (get_unix_prot(get_page_vprot(addr)) & PROT_READ)) {
        SIZE_T block_size =
            min(size - bytes_read, page_size - ((UINT_PTR)addr & page_mask));
 ...
 * exceptions can happen.
 */
NTSTATUS virtual_uninterrupted_write_memory(void *addr, const void *buffer,
                                            SIZE_T size) {
  BOOL has_write_watch = FALSE;
  sigset_t sigset;
  NTSTATUS ret;
...
#ifndef __APPLE__ /* On macOS, we still want to free some of low memory, for   \
                     OpenGL resources */
  if (user_space_limit > (void *)limit_2g)
    return;
#endif
...
void virtual_set_large_address_space(void) {
  if (is_win64) {
    ...
    else if ((main_image_info.DllCharacteristics &
              IMAGE_DLLCHARACTERISTICS_HIGH_ENTROPY_VA) &&
             (main_image_info.DllCharacteristics &
              IMAGE_DLLCHARACTERISTICS_DYNAMIC_BASE))
      free_reserved_memory(0, (char *)0x7ffe0000);
#endif
  } else {
    if (!(main_image_info.ImageCharacteristics &
          IMAGE_FILE_LARGE_ADDRESS_AWARE))
      return;
 ...
 */
static NTSTATUS allocate_virtual_memory(void **ret, SIZE_T *size_ptr,
                                        ULONG type, ULONG protect,
                                        ULONG_PTR limit_low,
                                        ULONG_PTR limit_high, ULONG_PTR align,
                                        ULONG attributes) {
  void *base;
  unsigned int vprot;
  ...
  if (is_beyond_limit(0, size, working_set_limit))
    return STATUS_WORKING_SET_LIMIT_RANGE;
...
NTSTATUS WINAPI NtAllocateVirtualMemory(HANDLE process, PVOID *ret,
                                        ...
                                        ULONG type, ULONG protect) {
  static const ULONG type_mask =
      MEM_COMMIT | MEM_RESERVE | MEM_TOP_DOWN | MEM_WRITE_WATCH | MEM_RESET;
  ULONG_PTR limit;

  TRACE("%p %p %08lx %x %08x\n", process, *ret, *size_ptr, (int)type,
        (int)protect);
  ...
  if (zero_bits > 21 && zero_bits < 32)
    return STATUS_INVALID_PARAMETER_3;
  if (zero_bits > 32 && zero_bits < granularity_mask)
    return STATUS_INVALID_PARAMETER_3;
#ifndef _WIN64
  if (!is_old_wow64() && zero_bits >= 32)
    return STATUS_INVALID_PARAMETER_3;
#endif
  if (type & ~type_mask)
    return STATUS_INVALID_PARAMETER;
  ...
  if (process != NtCurrentProcess()) {
    ...
    if (result.virtual_alloc.status == STATUS_SUCCESS) {
      ...
      *size_ptr = result.virtual_alloc.size;
      ...
      WARN("cross-process allocation failed, process=%p base=%p size=%08lx "
           "status=%08x",
...
static NTSTATUS get_extended_params(const MEM_EXTENDED_PARAMETER *parameters,
                                    ULONG count, ULONG_PTR *limit_low,
                                    ULONG_PTR *limit_high, ULONG_PTR *align,
  ...
  for (i = 0; i < count; ++i) {
    if (parameters[i].Type >= 32)
      return STATUS_INVALID_PARAMETER;
    if (present & (1u << parameters[i].Type))
      return STATUS_INVALID_PARAMETER;
    ...
    case MemExtendedParameterAddressRequirements: {
      ...
      if (r->Alignment) {
        if ((r->Alignment & (r->Alignment - 1)) ||
            r->Alignment - 1 < granularity_mask) {
      ...
      if (r->HighestEndingAddress) {
        *limit_high = (ULONG_PTR)r->HighestEndingAddress;
        if (*limit_high > limit || *limit_high <= *limit_low ||
 ...
 */
NTSTATUS WINAPI NtAllocateVirtualMemoryEx(HANDLE process, PVOID *ret,
  ...
  TRACE("%p %p %08lx %x %08x %p %u\n", process, *ret, *size_ptr, (int)type,
        (int)protect, parameters, (int)count);
  ...
  if (type & ~type_mask)
    return STATUS_INVALID_PARAMETER;
  if (*ret && (align || limit_low || limit_high))
    return STATUS_INVALID_PARAMETER;
  if (!*size_ptr)
    return STATUS_INVALID_PARAMETER;
  ...
  return allocate_virtual_memory(ret, size_ptr, type, protect, limit_low,
                                 limit_high, align, attributes);
}
...
NTSTATUS WINAPI NtFreeVirtualMemory(HANDLE process, PVOID *addr_ptr,
  ...
  if (!base) {
    ...
    if (addr == (void *)1 && !size && type == MEM_RELEASE)
      virtual_release_address_space();
    else
#endif
      status = STATUS_INVALID_PARAMETER;
  } else if (!(view = find_view(base, 0)))
    status = STATUS_MEMORY_NOT_ALLOCATED;
  else if (!is_view_valloc(view))
    status = STATUS_INVALID_PARAMETER;
  else if (!size && base != view->base)
 ...
 */
NTSTATUS WINAPI NtProtectVirtualMemory(HANDLE process, PVOID *addr_ptr,
  ...
  if ((view = find_view(base, size))) {
    ...
    if (get_committed_size(view, base, ~(size_t)0, &vprot, VPROT_COMMITTED) >=
            size &&
        (vprot & VPROT_COMMITTED)) {
...
static unsigned int fill_basic_memory_info(const void *addr,
  ...
  if (is_beyond_limit(base, 1, working_set_limit))
    return STATUS_INVALID_PARAMETER;
  ...
  if (!ptr) {
     ...
     * so that the app doesn't believe it's fully available */
    {
      ...
      LIST_FOR_EACH_ENTRY(area, &reserved_areas, struct reserved_area, entry) {
        ...
        if (area_start < alloc_end)
          info->RegionSize = area_start - base;
        break;
      }
      if (!in_reserved) {
    ...
    info->AllocationBase = alloc_base;
    info->RegionSize =
        get_committed_size(view, base, ~(size_t)0, &vprot, ~VPROT_WRITEWATCH);
    info->State = (vprot & VPROT_COMMITTED) ? MEM_COMMIT : MEM_RESERVE;
    info->Protect =
        (vprot & VPROT_COMMITTED) ? get_win32_prot(vprot, view->protect) : 0;
    info->AllocationProtect = get_win32_prot(view->protect, view->protect);
    if (view->protect & SEC_IMAGE)
      info->Type = MEM_IMAGE;
    else if (view->protect & (SEC_FILE | SEC_RESERVE | SEC_COMMIT))
...
static unsigned int get_basic_memory_info(HANDLE process, LPCVOID addr,
  ...
  if (process != NtCurrentProcess()) {
    ...
    if (result.virtual_query.status == STATUS_SUCCESS) {
      ...
      info->AllocationBase =
          wine_server_get_ptr(result.virtual_query.alloc_base);
...
static unsigned int get_memory_region_info(HANDLE process, LPCVOID addr,
  ...
  if ((status = fill_basic_memory_info(addr, &basic_info)))
    return status;
...
static void
init_fill_working_set_info_data(struct fill_working_set_info_data *d,
                                char *end) {
...
static void
free_fill_working_set_info_data(struct fill_working_set_info_data *d) {
  if (d->vmentries)
...
static void fill_working_set_info(struct fill_working_set_info_data *d,
                                  struct file_view *view, BYTE vprot,
                                  struct working_set_info_ref *ref,
                                  SIZE_T count,
  ...
  for (i = 0; i < count; ++i) {
    ...
    for (j = 0; j < d->vmentry_count; j++) {
      if (d->vmentries[j].kve_start <= (ULONG_PTR)p->VirtualAddress &&
          (ULONG_PTR)p->VirtualAddress <= d->vmentries[j].kve_end) {
        entry = &d->vmentries[j];
        break;
      }
    }
    ...
    p->VirtualAttributes.Valid = !(vprot & VPROT_GUARD) && (vprot & 0x0f) &&
                                 entry && entry->kve_type != KVME_TYPE_SWAP;
    p->VirtualAttributes.Shared = !is_view_valloc(view);
    if (p->VirtualAttributes.Shared && p->VirtualAttributes.Valid)
      p->VirtualAttributes.ShareCount = 1; /* FIXME */
    if (p->VirtualAttributes.Valid)
      p->VirtualAttributes.Win32Protection =
...
init_fill_working_set_info_data(struct fill_working_set_info_data *d,
...
#ifdef O_CLOEXEC
  if ((pagemap_fd = open("/proc/self/pagemap", O_RDONLY | O_CLOEXEC, 0)) ==
          -1 &&
      errno == EINVAL)
#endif
  ...
  if (pagemap_fd == -1)
    WARN("unable to open /proc/self/pagemap\n");
  else
    fcntl(pagemap_fd, F_SETFD,
          FD_CLOEXEC); /* in case O_CLOEXEC isn't supported */
}
...
static void fill_working_set_info(struct fill_working_set_info_data *d,
  ...
  for (i = 0; i < count; ++i) {
    ...
    if (page >= d->buffer_start + d->buffer_len) {
      ...
      if (pagemap_fd != -1) {
        d->buffer_len =
            pread(pagemap_fd, d->pm_buffer, len, page * sizeof(pagemap));
        if (d->buffer_len != len) {
          d->buffer_len = max(d->buffer_len, 0);
          memset(d->pm_buffer + d->buffer_len / sizeof(pagemap), 0,
                 len - d->buffer_len);
        }
      }
    ...
    p->VirtualAttributes.Valid =
        !(vprot & VPROT_GUARD) && (vprot & 0x0f) && (pagemap >> 63);
    p->VirtualAttributes.Shared =
        !is_view_valloc(view) && ((pagemap >> 61) & 1);
    if (p->VirtualAttributes.Shared && p->VirtualAttributes.Valid)
      p->VirtualAttributes.ShareCount = 1; /* FIXME */
    if (p->VirtualAttributes.Valid)
      p->VirtualAttributes.Win32Protection =
...
static NTSTATUS get_working_set_ex(HANDLE process, LPCVOID addr,
  ...
  if (process != NtCurrentProcess()) {
    FIXME("(process=%p,addr=%p) Unimplemented information class: "
  ...
  if (count > ARRAY_SIZE(ref_buffer))
    ref = malloc(count * sizeof(*ref));
  for (i = 0; i < count; ++i) {
    ref[i].orig_index = i;
  ...
  while (view && (char *)view->base > start) {
    ...
    if (!prev_view || (char *)prev_view->base + prev_view->size <= start)
      break;
    view = prev_view;
  }
  ...
  while (view && (char *)view->base < end) {
    if (start < (char *)view->base)
      start = view->base;
    while (r != ref + count && r->addr < start)
      ++r;
    while (start != (char *)view->base + view->size && r != ref + count &&
           r->addr < (char *)view->base + view->size) {
      start += get_committed_size(view, start, end - start, &vprot,
                                  ~VPROT_WRITEWATCH);
      i = 0;
      while (r + i != ref + count && r[i].addr < start)
        ++i;
...
static unsigned int get_memory_section_name(HANDLE process, LPCVOID addr,
  ...
  SERVER_START_REQ(get_mapping_filename) {
    ...
    if (!status || status == STATUS_BUFFER_OVERFLOW) {
      if (ret_len)
        *ret_len = sizeof(*info) + reply->len + sizeof(WCHAR);
      if (len < sizeof(*info))
...
static unsigned int get_memory_image_info(HANDLE process, LPCVOID addr,
  ...
  if (status == STATUS_NOT_MAPPED_VIEW) {
    ...
    status = get_basic_memory_info(process, addr, &basic_info,
                                   sizeof(basic_info), NULL);
    if (status || basic_info.State == MEM_FREE)
...
NTSTATUS WINAPI NtQueryVirtualMemory(HANDLE process, LPCVOID addr,
  ...
  case MemoryWineUnixWow64Funcs:
    if (len != sizeof(unixlib_handle_t))
      return STATUS_INFO_LENGTH_MISMATCH;
    if (process == GetCurrentProcess()) {
      ...
      status = get_builtin_unix_funcs(
          module, info_class == MemoryWineUnixWow64Funcs, &funcs);
      if (!status)
        *(unixlib_handle_t *)buffer = (UINT_PTR)funcs;
      return status;
    }
 ...
 */
NTSTATUS WINAPI NtLockVirtualMemory(HANDLE process, PVOID *addr, SIZE_T *size,
                                    ULONG unknown) {
  unsigned int status = STATUS_SUCCESS;
 ...
 */
NTSTATUS WINAPI NtUnlockVirtualMemory(HANDLE process, PVOID *addr, SIZE_T *size,
                                      ULONG unknown) {
  unsigned int status = STATUS_SUCCESS;
 ...
 */
NTSTATUS WINAPI NtMapViewOfSection(HANDLE handle, HANDLE process,
                                   PVOID *addr_ptr, ULONG_PTR zero_bits,
                                   SIZE_T commit_size,
                                   const LARGE_INTEGER *offset_ptr,
                                   SIZE_T *size_ptr, SECTION_INHERIT inherit,
  ...
  TRACE("handle=%p process=%p addr=%p off=%s size=0x%lx alloc_type=0x%x "
        "access=0x%x\n",
        handle, process, *addr_ptr, wine_dbgstr_longlong(offset.QuadPart),
        *size_ptr, (int)alloc_type, (int)protect);
  ...
  if (*addr_ptr && zero_bits && zero_bits < 32 &&
    ...
    return STATUS_INVALID_PARAMETER_4;
  if (*addr_ptr && zero_bits >= 32 && (((UINT_PTR)*addr_ptr) & ~zero_bits))
    return STATUS_INVALID_PARAMETER_4;
  ...
  return virtual_map_section(handle, addr_ptr, 0,
                             get_zero_bits_limit(zero_bits), commit_size,
 ...
 */
NTSTATUS WINAPI NtMapViewOfSectionEx(
    HANDLE handle, HANDLE process, PVOID *addr_ptr,
  ...
  if (align)
    return STATUS_INVALID_PARAMETER;
  if (*addr_ptr && (limit_low || limit_high))
    return STATUS_INVALID_PARAMETER;
...
static NTSTATUS unmap_view_of_section(HANDLE process, PVOID addr, ULONG flags) {
  ...
  if (process != NtCurrentProcess()) {
    ...
    status = server_queue_process_apc(process, &call, &result);
    if (status == STATUS_SUCCESS)
      status = result.unmap_view.status;
    return status;
  }
  ...
  if (!status) {
    ...
    if (flags & MEM_PRESERVE_PLACEHOLDER)
      free_pages_preserve_placeholder(view, view->base, view->size);
    else
      delete_view(view);
  } else
    FIXME("failed to unmap %p %x\n", view->base, status);
done:
  server_leave_uninterrupted_section(&virtual_mutex, &sigset);
  return status;
}
 ...
 */
NTSTATUS WINAPI NtUnmapViewOfSectionEx(HANDLE process, PVOID addr,
                                       ULONG flags) {
  static const ULONG type_mask =
      MEM_UNMAP_WITH_TRANSIENT_BOOST | MEM_PRESERVE_PLACEHOLDER;

  if (flags & ~type_mask) {
    WARN("Unsupported flags %#x.\n", (int)flags);
    return STATUS_INVALID_PARAMETER;
  }
  if (flags & MEM_UNMAP_WITH_TRANSIENT_BOOST)
    FIXME("Ignoring MEM_UNMAP_WITH_TRANSIENT_BOOST.\n");
 ...
 */
void virtual_fill_image_information(const struct pe_image_info *pe_info,
                                    SECTION_IMAGE_INFORMATION *info) {
  info->TransferAddress =
      wine_server_get_ptr(pe_info->base + pe_info->entry_point);
...
NTSTATUS WINAPI NtQuerySection(HANDLE handle, SECTION_INFORMATION_CLASS class,
  ...
  case SectionBasicInformation:
    if (size < sizeof(SECTION_BASIC_INFORMATION))
      return STATUS_INFO_LENGTH_MISMATCH;
    break;
  case SectionImageInformation:
    if (size < sizeof(SECTION_IMAGE_INFORMATION))
      return STATUS_INFO_LENGTH_MISMATCH;
    break;
  default:
...
NTSTATUS WINAPI NtFlushVirtualMemory(HANDLE process, LPCVOID *addr_ptr,
  ...
  server_enter_uninterrupted_section(&virtual_mutex, &sigset);
  if (!(view = find_view(addr, *size_ptr)))
    status = STATUS_INVALID_PARAMETER;
  else {
    if (!*size_ptr)
      *size_ptr = view->size;
    *addr_ptr = addr;
#ifdef MS_ASYNC
    if (msync(addr, *size_ptr, MS_ASYNC))
 ...
 */
NTSTATUS WINAPI NtGetWriteWatch(HANDLE process, ULONG flags, PVOID base,
                                SIZE_T size, PVOID *addresses, ULONG_PTR *count,
                                ULONG *granularity) {
  NTSTATUS status = STATUS_SUCCESS;
  sigset_t sigset;
  ...
  if (is_write_watch_range(base, size)) {
    ...
    while (pos < *count && addr < end) {
      if (!(get_page_vprot(addr) & VPROT_WRITEWATCH))
        addresses[pos++] = addr;
      addr += page_size;
    }
    if (flags & WRITE_WATCH_FLAG_RESET)
      reset_write_watches(base, addr - (char *)base);
    *count = pos;
 ...
 */
NTSTATUS WINAPI NtReadVirtualMemory(HANDLE process, const void *addr,
 ...
 */
NTSTATUS WINAPI NtWriteVirtualMemory(HANDLE process, void *addr,
...
static NTSTATUS prefetch_memory(HANDLE process, ULONG_PTR count,
  ...
  if (!once++) {
    FIXME("(process=%p,flags=%u) "
          "NtSetInformationVirtualMemory(VmPrefetchInformation) partial stub\n",
          process, (int)flags);
  }
...
static NTSTATUS set_dirty_state_information(ULONG_PTR count,
  ...
  for (i = 0; i < count; i++) {
    ...
    SIZE_T size =
        ROUND_SIZE(addresses[i].VirtualAddress, addresses[i].NumberOfBytes);
 ...
 */
NTSTATUS WINAPI NtSetInformationVirtualMemory(
    HANDLE process, VIRTUAL_MEMORY_INFORMATION_CLASS info_class,
    ULONG_PTR count, PMEMORY_RANGE_ENTRY addresses, PVOID ptr, ULONG size) {
  TRACE("(%p, info_class=%d, %lu, %p, %p, %u)\n", process, info_class, count,
        addresses, ptr, (int)size);
 ...
 */
NTSTATUS WINAPI NtFlushInstructionCache(HANDLE handle, const void *addr,
                                        SIZE_T size) {
 ...
 */
NTSTATUS WINAPI NtCreatePagingFile(UNICODE_STRING *name,
                                   LARGE_INTEGER *min_size,
                                   LARGE_INTEGER *max_size,
                                   LARGE_INTEGER *actual_size) {
  FIXME("(%s %p %p %p) stub\n", debugstr_us(name), min_size, max_size,
        actual_size);
  return STATUS_SUCCESS;
}
 ...
 */
NTSTATUS WINAPI NtWow64AllocateVirtualMemory64(HANDLE process, ULONG64 *ret,
                                               ULONG64 zero_bits,
                                               ULONG64 *size_ptr, ULONG type,
                                               ULONG protect) {
  void *base;
  SIZE_T size;
  unsigned int status;

  TRACE("%p %s %s %x %08x\n", process, wine_dbgstr_longlong(*ret),
        wine_dbgstr_longlong(*size_ptr), (int)type, (int)protect);
  ...
  status =
      NtAllocateVirtualMemory(process, &base, zero_bits, &size, type, protect);
  if (!status) {
    *ret = (ULONG_PTR)base;
    *size_ptr = size;
  }
 ...
 */
NTSTATUS WINAPI NtWow64ReadVirtualMemory64(HANDLE process, ULONG64 addr,
                                           void *buffer, ULONG64 size,
                                           ULONG64 *bytes_read) {
  unsigned int status;
 ...
 */
NTSTATUS WINAPI NtWow64WriteVirtualMemory64(HANDLE process, ULONG64 addr,
                                            const void *buffer, ULONG64 size,
...
NTSTATUS WINAPI NtWow64GetNativeSystemInformation(
    SYSTEM_INFORMATION_CLASS class, void *info, ULONG len, ULONG *retlen) {
  NTSTATUS status;
...
NTSTATUS WINAPI NtWow64IsProcessorFeaturePresent(UINT feature) {
  return feature < PROCESSOR_FEATURE_MAX &&
